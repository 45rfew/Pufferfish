  ...
  switch (event.name){
    case "ui_component_clicked":
      ...
      if (component.includes("upgrade")&&ship.alive){
        ...
        let i = parseInt(b); 
        let o = ship.custom.c1.cost[i].replace('EXP','');
        let g = parseInt(o);
        if (ship.custom.exp >= g){
          ship.custom.exp -= g;
          ...
          ship.setUIComponent({
            id:"oof",visible:true,position:[30,28,100,100],
            components: [{type:"text",position:[0,0,40,40],value:"You do not have enough EXP to purchase this!",color:"#f99e9e"}]
          });     
          setTimeout(function(){ship.setUIComponent({id:"oof",visible:false});},2000);
        }
      } else 
      if (component.includes("weapon")&&ship.alive){
        ...
        let i = parseInt(b); 
        let c = [10,11,12];
        let o = ship.custom.c2.cost[i].replace('EXP','');
        let g = parseInt(o);
        let t = typeof parseInt(ship.custom.c2.cost[i]);
        if (g > 1000){
          if (ship.custom.exp >= g){
            ship.custom.exp -= g;
      ...
      if (component.includes("ability")&&ship.alive){
        ...
        let i = parseInt(b); 
        let o = ship.custom.c3.cost[i].replace('EXP','');
        let g = parseInt(o);
        if (ship.custom.exp >= g){
          ship.custom.exp -= g;
      ...
      if (component.includes("EXP")&&ship.alive){
        ...
        let i = parseInt(b); 
        let m = [1.2,1.5,2];
        let o = ship.custom.c4.cost[i].replace('EXP','');
        let g = parseInt(o);
        if (ship.custom.exp >= g){        
          ship.custom.exp -= g;
    ...
    case "alien_destroyed":
      ...
      switch (alien.level){
        case 0: killer.custom.exp += Math.round(40*killer.custom.mexp); break;
        case 1: killer.custom.exp += Math.round(60*killer.custom.mexp); break;
        case 2: killer.custom.exp += Math.round(120*killer.custom.mexp); break;
        case 3: killer.custom.exp += Math.round(1750*killer.custom.mexp); break;
      }
      break;
...
Odyssey2.model = 2;
Odyssey2.size = 4*1.2;
Odyssey2.typespec.specs.ship.speed = [55,55];
...
Odyssey3.model = 3;
Odyssey3.size = 4.8*1.2;
Odyssey3.typespec.specs.ship.speed = [60,60];
...
Shadow_X_33.typespec.specs.generator.capacity = [900,900];
Shadow_X_33.typespec.specs.generator.reload = [200,20];
Shadow_X_33.typespec.lasers[0] = {
  "x":0,"y":-7.8,"z":0,"angle":0,
  "damage":[800,800],"rate":1,"type":1,
